...
- **部屋番号**：`888999 / 777888 / 666777 / 555666`（**任意・初期値は指定なし**）。「その他（手動入力）」で自由入力も可能、「指定なし」で空に戻せます（候補には追加されません）。
...
> **部屋番号は作成VCのチャンネルステータスに `id: 888999` の形式で表示**されます（指定なしの場合は付きません）。
> 募集通知には作成者・ゲーム・目的・参加人数・対象者・対象ランク・部屋番号・ひとこと・VCリンクが表示され、
> 「メンバー」ロールへのメンションと 👍 リアクションが付きます。
> **VCが閉じると、その募集通知メッセージに 🏁 リアクションが付いて終了が分かります。**
> 連投による通知スパムを防ぐため、募集作成は**同一ユーザー10分あたり5回まで**に制限されます。

...
「簡易募集」チャンネルに設置したボタンを押すと、自分だけに見える入力フォーム（ドロップダウン）が開きます。
ゲーム・人数・対象者・対象ランク・ひとことを選んで **「VCを作成」** を押すと、**専用のボイスチャットが自動作成**され、
**「募集通知」チャンネルに募集が投稿**されます。募集主は作成したVCへ自動で移動します（既にVCに居る場合）。
**参加者が全員退出すると、そのVCは自動的に削除**されます。
...
```
===== 簡単募集（ベータ版） =====   ← カテゴリ
├─ #簡易募集      … ボタン＆入力フォーム（入力内容は本人にのみ表示・履歴に残らない）
...
- **募集するゲーム**：`GGST / GGST（Switch）/ スト６ / シャドバ / デュエプレ / TRPG / 雀魂`（初期値）。
  一覧に無ければ「その他（手動入力）」で入力でき、入力した名前は以降ドロップダウンに自動追加されます。
- **参加人数**：`2〜10人 / 無制限`。選んだ人数が作成VCの入室上限にも反映されます（無制限=上限なし）。
- **対象者**：`制限なし / イツメン / サーバー初心者`。
- **対象ランク**：`制限なし / ゴールド以下 / プラチナ以下 / ダイヤ以下`。
- **ひとこと**：自由入力（任意・最大100文字）。募集通知に表示されます。

> ℹ️ 「対象者」「対象ランク」は募集通知に**ラベル表示**して参加者を誘導するもので、入室そのものは制限しません。
> 作成されるVCのタイトルは `🎙️ ゲーム / 人数 / 対象者 / 対象ランク`、募集通知には作成者・ゲーム・参加人数・
> 対象者・対象ランク・ひとこと・VCリンクが表示されます。

```bash
# 初回セットアップ（管理者）— これ1つで完了
/vc setup

# 以降は誰でも「簡易募集」チャンネルの「VCを立てる」ボタンから募集できます
```

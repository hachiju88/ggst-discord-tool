...
募集チャンネルに設置したボタンを押すと、自分だけに見える入力フォーム（ドロップダウン）が開きます。
ゲーム・人数・対象者・対象ランクを選んで **「VCを作成」** を押すと、**専用のボイスチャットが自動作成**され、
募集チャンネルに告知が投稿されます。**参加者が全員退出すると、そのVCは自動的に削除**されます。

> ⚠️ VCの参加検知・自動削除には Discord Developer Portal の **Server Voice States**（Presence とは別）が
> 使われます。Bot には **チャンネルの管理** 権限が必要です。

| コマンド | サブコマンド | 説明 |
|---------|------------|------|
| `/vc` | `set-category` | 募集VCを作成するカテゴリを設定（管理者） |
...
- **募集するゲーム**：`GGST / 雑談 / スト６ / シャドウバース`（初期値）。一覧に無ければ「その他（手動入力）」で
  入力でき、入力した名前は以降ドロップダウンに自動追加されます。
- **参加人数**：`2〜10人 / 無制限`。選んだ人数が作成VCの入室上限にも反映されます（無制限=上限なし）。
- **対象者**：`制限なし / イツメン向け（VC参加3回以上）/ サーバー初心者向け（VC参加3回未満）`。
- **対象ランク**：`制限なし / ゴールド以下 / プラチナ以下 / ダイヤ以下`。

> ℹ️ 「対象者」「対象ランク」は募集告知に**ラベル表示**して参加者を誘導するもので、入室そのものは制限しません。
> VC参加回数は自動で記録され、告知の「募集主のVC参加: N回」表示に使われます（将来の自動化の土台）。

```bash
# 初回セットアップ（管理者）
/vc set-category category:🎙️募集VC   # 作成先カテゴリを指定
/vc panel                            # 募集チャンネルにボタンを設置

# 以降は誰でも「VCを立てる」ボタンから募集できます
```
